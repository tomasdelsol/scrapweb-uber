...
export async function modificarPlanes() {
    ...
    const modifiedDataPlans : CarInfo[] = jsonData.map(car => ({
        ...
        plans: car.plans.map(plan => ({
            ...
            planNames: plan.planNames.flatMap(name => {
                ...
                if (trimmedName === "UberX") return ["ByFy X", "ByFy Pets", "ByFy Envíos"];
                if (trimmedName === "Comfort") return ["ByFy Comfort"];
                if (trimmedName === "Encargo") return ["ByFy Moto", "ByFy Envíos Moto"];
                // Si no coincide con ninguno, devolver array vacío para filtrarlo
                return [];
            }),
        })),
    }));
